...
Now we know that python originally is a scripting language but over time it has exposed several programming paradigms like object-oriented programming, asynchronous programming, array-oriented programming, and functional programming which is also of great interest for Big Data professionals. That is because, through functional programming, the data can be manipulated by functions without having to maintain an external state. So, your code is always returning new data instead of manipulating data in place, uses anonymous functions, and avoids global variables. 
